...
public class NPC extends GameObject {
	...
	public void update(Game game) {
		...
		if(target!=null && room==target.room && getDistance(target)<200 )
			separationVector.set(steering);
		
		for(int i=0;i<LevelManager.players.size();i++){
			Player e=LevelManager.players.get(i);
			if(room==e.room){
				separationVector.add(avoidVector(e,400,200));
			}
		}
		if(target!=null){
	...
	protected void handleObjectCollision(GameObject object,String direction){
		...
		if(object instanceof Bullet && ((Bullet) object).damageNPC){
			...
			if(health < 0){
				SoundManager.mixedSound("enemyDeath");
				die();
			}
